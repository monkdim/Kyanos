...
*calls itself* was refused by name at the time: the closure was built from the
values in scope and its own name was not one of them yet, which needs
by-reference capture. That arrived with the closed-over-`mut` entry below, and
the refusal is gone.
...
### A closed-over `mut` was captured by value, so a counter never counted
Fixed. A closure in a native build took a copy of every free variable when it
was made. A copy is right only if nothing ever writes the variable, and the
first thing anyone writes a closure for is a counter:
...
The closure incremented its own copy of `n` and the copy died with the call.
The same snapshot was wrong in the other direction too — a closure built
before a later assignment kept reading the old value:
...
show f()          -- run and --fast: 5;  cc: 0
```

A closure driving a `while` condition was worse than wrong. This loop never
terminates in a compiled binary -- `g()` increments its own copy, so the
condition never changes -- and it allocates until the kernel kills it, exit 137
...
fn whileloop() {
    ...
    let g = fn() { i += 1
        return i }
    while i < 3 { push(acc, g()) }
    return acc
}
```

And a nested `fn` that called itself was refused outright, with a message
saying native builds have no by-reference capture and the function should be
moved to the top level.

A name a closure both **sees and writes** now lives in a one-slot list, and
the closure captures that list rather than the value in it, so the closure and
the scope that made it look at the same slot. A one-slot list rather than a
new tag because the collector already traces a `List`, and a box is never
handed to a program: only the emitter makes one and only the emitter reads it.

Only such a name. Captured-but-never-assigned stays a plain copy, which is the
common case and costs nothing; assigned-but-never-captured stays an ordinary C
local, because nobody else is looking. A comprehension variable is scoped to
the comprehension and is never boxed, whatever it shadows. A self-recursive
nested `fn` falls out of the same mechanism — the box is declared empty, the
closure captures it, and the closure goes into the slot — so the refusal is
gone.

**The example was the place this was written down.** `examples/functions.clarity`
has printed a stuck counter for as long as `clarity cc` has compiled it, and
the suite said the examples were fine, because "fine" meant the compiler
produced C for them and nothing more. The examples check now runs every
example the interpreter can run and diffs the output; the FFI examples, which
have no interpreter counterpart, are compiled and skipped, and the number
compared is itself asserted so it cannot quietly fall to zero.
