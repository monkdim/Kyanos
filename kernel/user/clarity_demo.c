...
static Value cl_get_field(Value obj, const char* name){
  ...
  if(obj.t==T_OBJECT) m=(Map*)((Obj*)obj.o)->fields.o;   /* instance field map */
    ...
    char buf[160];
    if(obj.t==T_STR) snprintf(buf, sizeof buf, "RuntimeError: String has no property '%s'", name);
    else if(obj.t==T_LIST) snprintf(buf, sizeof buf, "RuntimeError: List has no property '%s'", name);
    else if(cl_is_num(obj)) snprintf(buf, sizeof buf, "RuntimeError: Number has no property '%s'", name);
    else snprintf(buf, sizeof buf, "TypeError: Cannot access property on %s", cl_type_of(obj).s);
    cl_throw(cl_str(cl_strdup(buf)));
    return cl_null();
  }
  for(long j=0;j<m->len;j++) if(strcmp(m->keys[j],name)==0) return m->vals[j];
